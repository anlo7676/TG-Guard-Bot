...
v1.13.1 按身份精简私聊首页和帮助：普通成员只显示个人功能，群管理员显示群设置，机器人管理员显示管理后台。私聊默认命令列表移除群管理命令；管理员通过首页入口管理群组。
...
- 私聊「自助验证 / 解除禁言」或 `/verify` 按 Telegram 当前状态查找自己被禁言的已授权群组，不依赖旧验证记录。支持广告处罚和管理员禁言；每人每群每小时最多新开 3 次，答对后解除禁言并记录，不发送欢迎语，不清空违规记录或黑名单。仅限仍在群内的成员，不解除封禁；未答对或超时不恢复权限。验证过程中发生新的管理员操作会终止本次验证，可在操作完成后重新验证。
