...
func (s *Store) CancelVerification(ctx context.Context, chat, user int64) error {
	_, e := s.DB.ExecContext(ctx, "UPDATE verification_sessions SET status='cancelled' WHERE chat_id=? AND user_id=? AND status IN ('pending','completing','expiring')", chat, user)
	return e
}
...
func (s *Store) ActiveVerification(ctx context.Context, chat, user int64) (Verification, error) {
	return scanVerification(s.DB.QueryRowContext(ctx, "SELECT "+verifyColumns+" FROM verification_sessions WHERE chat_id=? AND user_id=? AND status IN ('pending','completing','expiring') ORDER BY created_at DESC LIMIT 1", chat, user))
}
...
func (s *Store) DueVerifications(ctx context.Context) ([]Verification, error) {
	...
	rows, e := s.DB.QueryContext(ctx, "SELECT "+verifyColumns+" FROM verification_sessions WHERE status IN ('expiring','completing') AND next_attempt_at<=UTC_TIMESTAMP(6) ORDER BY next_attempt_at LIMIT 100")
	if e != nil {
		return nil, e
	}
	defer rows.Close()
...
func (s *Store) VerificationRetry(ctx context.Context, token string, cause error) error {
	_, e := s.DB.ExecContext(ctx, "UPDATE verification_sessions SET next_attempt_at=DATE_ADD(UTC_TIMESTAMP(6),INTERVAL LEAST(300,5*(recovery_attempts+1)) SECOND),recovery_attempts=recovery_attempts+1,last_error=? WHERE token=? AND status IN ('completing','expiring')", clip(cause.Error(), 500), token)
	return e
}
