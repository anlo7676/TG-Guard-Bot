...
应用入口通过 `service.New` 校验依赖，`runApplication` 统一管理启动和退出。审核计算拆到 `moderationEngine`，只依赖消费方定义的违规计数、Spam 计数和 AI 调用接口；Handler 直接使用本次审核返回值，避免二次查库。群统计采用类型化读模型。Service 仍是验证、菜单与处罚的应用协调层，并非每个数据库方法都再包装一层接口。

动态正则共用有界缓存（最多 2048 个模式，读写锁保护），配置验证和消息匹配复用编译结果。达到上限后清空并按需重建；不缓存群权限和授权决定，避免安全配置失效延迟。数据库为成员日志查询和群验证时间查询增加复合索引，现阶段保持单实例持有 MySQL 锁、实例内跨群并发。
