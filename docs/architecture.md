...
- 原始收件箱、审核文本和日志包含用户数据，部署方应设置访问权限和保留期。API 密钥不写数据库；未来支持 Web 配置 Provider 时再引入 KMS／信封加密及脱敏接口。
...
1. Vue／React Web 后台、管理员登录、按群授权、账号操作审计和完整国际化。
2. 图片验证码和 Turnstile/Mini App Provider；OCR／二维码／视觉审核。
3. 关键词链接按钮、语义相似 Spam、滑动窗口多档限流、可视化策略编辑器。
4. Provider 加密配置、实际供应商费用核算、AI 配额、模型回归数据集。
